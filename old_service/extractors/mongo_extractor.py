...
class MongoExtractor:
        ...
        try:
            ...
            for i, doc in enumerate(cursor):
                ...
                if fecha_hora is None or fecha_hora == '' or str(fecha_hora).lower() == 'null':
                    problematic_count += 1
                    print(f"PROBLEMA - Pedido {doc.get('id_pedido', 'unknown')}: fecha_hora = {repr(fecha_hora)}")
            ...
            print(f"Pedidos con fecha problemática: {problematic_count}")
            print(f"Coordenadas completas: {coordenadas_completas}/{len(pedidos)}")
            print(f"Coordenadas parciales: {coordenadas_parciales}/{len(pedidos)}")
            print(f"Sin coordenadas: {len(pedidos) - coordenadas_completas - coordenadas_parciales}/{len(pedidos)}")
    ...
    def extract_detalle_pedidos(self):
        ...
        try:
            ...
            print(f"{len(detalles)} detalles de pedidos extraídos de MongoDB")
            return detalles
